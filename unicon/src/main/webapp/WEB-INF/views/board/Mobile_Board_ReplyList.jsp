...
<script>
	function fn_MobileEnable(obj,rep_contentId,count){
	...
	function fn_modify_MobileReply(rep_boardNO, rep_contentId,rep_parentNO){
		 var form=document.createElement("form");
		 form.setAttribute("method","post");
		 form.setAttribute("action","${contextPath}/board/MobileUpdateReply.do");
	 ...
	 /* 버튼 display 껏다 켯다 */
	function fn_addMobileReply(boardNO) {
	...
	/* 대댓글 AJAX 구문 */
	function fn_insertMobileReply(boardNO, parentNO) {
		...
		$.ajax({
			url : '${contextPath}/board/MobileAddReply.do',
			type : 'post',
	...
	/* 댓글 삭제 */
    function MobileRemove(rep_boardNO,rep_parentNO) {
		 var chk = confirm("정말 삭제 할꺼니?");
		 if(chk) {
			 alert("댓글이 삭제되었습니다.")
			 location.href='MobileDeleteReply.do?rep_boardNO=' + rep_boardNO + "&rep_parentNO=" + rep_parentNO;
		 } else {
			 return false;
		 }
	} 
						...
						<c:forEach var="reply" items="${replyList}" varStatus="replyNum">
							...
							<div id="nickNameDropReply${replyNum.count }"></div>				
									...
									<c:if test="${session_id == reply.rep_id}">
										<td id="tr_btn_mod${replyNum.count}" align="right" style="vertical-align:middle;">
											<input class="btn btn-default btn-round" type="button" value="수정" onClick="fn_MobileEnable(this.form,'r_content${replyNum.count}','${replyNum.count}')">
											<input class="btn btn-default btn-round" type="button" value="삭제" onClick="MobileRemove('${reply.rep_boardNO}','${board.boardNO}')">										
											<input type="hidden" name="rep_parentNO" value="${board.boardNO}">
										</td>
									</c:if>
								</td>
								<td id="tr_btn_save${replyNum.count}" class="tr_btn_save" align="right">
									<input class="btn btn-default btn-round" type="button" value="저장" onClick="fn_modify_MobileReply('${reply.rep_boardNO}','r_content${replyNum.count}','${board.boardNO}')">
									<input class="btn btn-default btn-round" type="button" value="취소" onClick="backToList(this.form)">
								</td>
								<td align="right" style="vertical-align:middle;">
									<input class="btn btn-default btn-round" type="button" id="reply_btn${reply.rep_boardNO}" value="대댓글 등록" onClick="fn_addMobileReply(${reply.rep_boardNO})" />
							...
							<tr id="d_reply${reply.rep_boardNO}" style="display:none;" align="left">
								<td colspan="4">
									...
									onClick="fn_insertMobileReply(${reply.rep_boardNO},${board.boardNO})" style="height:40px"/>
								</td>
							</tr>
						</c:forEach>
					</c:when>
