...
<script>
   function fn_FreeEnable(obj,rep_contentId,count){
   ...
   function fn_modify_FreeReply(rep_boardNO, rep_contentId,rep_parentNO){
       var form=document.createElement("form");
       form.setAttribute("method","post");
       form.setAttribute("action","${contextPath}/board/FreeUpdateReply.do");
   ...
   /* 버튼 display 껏다 켯다 */
   function fn_addFreeReply(boardNO) {
   ...
   /* 대댓글 AJAX 구문 */
   function fn_insertFreeReply(boardNO, parentNO) {
      ...
      $.ajax({
         url : '${contextPath}/board/FreeAddReply.do',
         type : 'post',
   ...
   /* 댓글 삭제 */
    function FreeRemove(rep_boardNO,rep_parentNO) {
       var chk = confirm("정말 삭제 할꺼니?");
       if(chk) {
          alert("댓글이 삭제되었습니다.")
          location.href='FreeDeleteReply.do?rep_boardNO=' + rep_boardNO + "&rep_parentNO=" + rep_parentNO;
       } else {
          return false;
       }
   } 
...
<body>
   <div class="container">
      <form name="repForm">
         <table class="table">
            <c:choose>
               <c:when test="${empty FreeReplyList}">
                  <tr>
                     <td align="center">
                        <p>
               ...
               <c:when test="${FreeReplyList != null}">
                  <c:forEach var="reply" items="${FreeReplyList}" varStatus="replyNum">
                     <tr align="center">                        
                          ...
                          <div class="dropdown">
                           ...
                           <c:if test="${session_id == reply.rep_id}">
                              <td id="tr_btn_mod${replyNum.count}" align="right" style="vertical-align:middle;">
                                 <input class="btn btn-default btn-round" type="button" value="수정" onClick="fn_FreeEnable(this.form,'r_content${replyNum.count}','${replyNum.count}')">
                                 <input class="btn btn-default btn-round" type="button" value="삭제" onClick="FreeRemove('${reply.rep_boardNO}','${board.boardNO}')">                              
                                 <input type="hidden" name="rep_parentNO" value="${board.boardNO}">
                              </td>
                           </c:if>
                        </td>
                        <td id="tr_btn_save${replyNum.count}" class="tr_btn_save" align="right">
                           <input class="btn btn-default btn-round" type="button" value="저장" onClick="fn_modify_FreeReply('${reply.rep_boardNO}','r_content${replyNum.count}','${board.boardNO}')">
                           <input class="btn btn-default btn-round" type="button" value="취소" onClick="backToList(this.form)">
                        </td>
                        <td align="right" style="vertical-align:middle;">
                           <input class="btn btn-default btn-round" type="button" id="reply_btn${reply.rep_boardNO}" value="대댓글 등록" onClick="fn_addFreeReply(${reply.rep_boardNO})" />
                     ...
                     <tr id="d_reply${reply.rep_boardNO}" style="display:none;" align="left">
                        <td colspan="4">
                           ...
                           onClick="fn_insertFreeReply(${reply.rep_boardNO}, ${board.boardNO})" style="height:40px"/>
                        </td>
                     </tr>
                  </c:forEach>
               </c:when>
