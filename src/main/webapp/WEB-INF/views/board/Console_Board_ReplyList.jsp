...
<script>
	function fn_ConsoleEnable(obj,rep_contentId,count){
	...
	/* 댓글수정 취소시 다시 돌아가기 (웨 안뒈...) */
	/* function moveView(rep_parentNO){
	   obj.action="${contextPath}/board/viewArticle2.do?boardNO=" + rep_parentNO;
	   obj.submit();
	   } */ 
	
	function fn_modify_ConsoleReply(rep_boardNO, rep_contentId,rep_parentNO){
		 var form=document.createElement("form");
		 form.setAttribute("method","post");
		 form.setAttribute("action","${contextPath}/board/ConsoleUpdateReply.do");
	...
	/* 버튼 display 껏다 켯다 */
	function fn_addConsoleReply(boardNO) {
	...
	/* 대댓글 AJAX 구문 */
	function fn_insertConsoleReply(boardNO, parentNO) {
		...
		$.ajax({
			url : '${contextPath}/board/ConsoleAddReply.do',
			type : 'post',
			data : {"rep_boardNO" : boardNO, "rep_content" : rep_content, "rep_parentNO" : parentNO, "daedatNO" : daedatNO },
			success:function(data){
				location.reload();
				//location.href="${contextPath}/board/ConsoleViewArticle.do?boardNO=" + parentNO;
			}
		});
	}
	
	/* 댓글 삭제 */
    function ConsoleRemove(rep_boardNO,rep_parentNO) {
		 var chk = confirm("정말 삭제 할꺼니?");
		 if(chk) {
			 alert("댓글이 삭제되었습니다.")
			 location.href='ConsoleDeleteReply.do?rep_boardNO=' + rep_boardNO + "&rep_parentNO=" + rep_parentNO;
		 } else {
			 return false;
		 }
	} 
						...
						<c:forEach var="reply" items="${replyList}" varStatus="replyNum">
							...
							<div id="nickNameDropReply${replyNum.count }"></div>				
									...
									<c:if test="${session_id == reply.rep_id}">
										<td id="tr_btn_mod${replyNum.count}" align="right" style="vertical-align:middle;">
											<input class="btn btn-default btn-round" type="button" value="수정" onClick="fn_ConsoleEnable(this.form,'r_content${replyNum.count}','${replyNum.count}')">
											<input class="btn btn-default btn-round" type="button" value="삭제" onClick="ConsoleRemove('${reply.rep_boardNO}','${board.boardNO}')">										
											<input type="hidden" name="rep_parentNO" value="${board.boardNO}">
										</td>
									</c:if>
								</td>
								<td id="tr_btn_save${replyNum.count}" class="tr_btn_save" align="right">
									<input class="btn btn-default btn-round" type="button" value="저장" onClick="fn_modify_ConsoleReply('${reply.rep_boardNO}','r_content${replyNum.count}','${board.boardNO}')">
									<input class="btn btn-default btn-round" type="button" value="취소" onClick="backToList(this.form)">
								</td>
								<td align="right" style="vertical-align:middle;">
									<input class="btn btn-default btn-round" type="button" id="reply_btn${reply.rep_boardNO}" value="대댓글 등록" onClick="fn_addConsoleReply(${reply.rep_boardNO})" />
							...
							<tr id="d_reply${reply.rep_boardNO}" style="display:none;" align="left">
								<td colspan="4">
									...
									onClick="fn_insertConsoleReply(${reply.rep_boardNO},${reply.rep_parentNO})" style="height:40px"/>
								</td>
							</tr>
						</c:forEach>
					</c:when>
